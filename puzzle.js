...
var p=setup.puzzle_wiregamer,freeslot=[],size=[],m=[],o,f=document.getElementById("wiregamer");
ceation_slots();
function ceation_slots(){
    ...
    if(p.fill){wiregamer_resize();window.addEventListener('resize',wiregamer_resize,true);}
    o=1;
...
function check_slots(){
    ...
    for(var y=0;y<=p.grid[1];y++){
        ...
        // alert after game end it runs at the end of the game
    }if(check==o){setTimeout(()=>{ alert('You Win you are true GAMER') },((p.time)?p.time*1000:0));} 
    // }if(check==o){setTimeout(()=>{ 
        
    //     var container = document.getElementById("won");
    //     var won = document.createElement("img");
    //     won.src = "won.gif";
    //     won.alt = "wining celibration image gif";
    //     won.title = "wining celibration image";
    //     container.appendChild(won);

    //  },((p.time)?p.time*1000:0));} 
}

function wiregamer_resize(){
